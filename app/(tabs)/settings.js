...
const App = () => {
  ...
  const updateCalculationMethod = useCalculationMethodStore(
    (state) => state.updateCalculationMethod
  )
  ...
  const updateCalculationMadhab = useCalculationMadhab(
    (state) => state.updateCalculationMadhab
  )
  return (
    <>
      <YStack paddingVertical="$4" paddingHorizontal="$4">
        <H1 color="black" marginBottom="$5">
          Settings
        </H1>
      ...
      >
        ...
        >
          <Button
            size="$6"
            circular
            icon={<ChevronsDown size={24} color="white" />}
